...
///--------------------------10 констант-------------------
        ...
        public static void RandomT(HashMap<Integer, ArrayList<Double>> pointsX, HashMap<Integer, ArrayList<Double>> pointsY, int maxIterNum) {
            ...
            for (int i = 0; i < maxIterNum; ++i) {
                ...
                for (int j = 0; j < 3; j++) {
                 //  cl = (Integer) randNumber.nextInt(j);

                   // System.out.println("cl= " + Math.abs(cl));
                    // pi -- вероятность, с которой выбираем преобразование Ti.
                    System.out.println("(sxx.get(i) + sxx.get(j)) * 0.5 " +((sxx.get(i) + sxx.get(j)) * 0.5)+"(syy.get(i) + syy.get(j)) * 0.5 "+((syy.get(i) + syy.get(j)) * 0.5));
                    sxx1.add((sxx.get(i) + sxx.get(j)) * 0.5);
                    syy1.add((syy.get(i) + syy.get(j)) * 0.5);
                }
                i++;
                pointsX.put(i, sxx1);
                pointsY.put(i, syy1);
